...
export default function App() {
  ...
  const showAddMenu = (e: React.MouseEvent) => {
    ...
    const items = [
      ...
      { label: '添加定时提醒', icon: menuIcons.reminder, onClick: addReminderItem },
      { divider: true },
  ...
  const showCardMenu = (e: React.MouseEvent, item: GridItem) => {
    ...
    if (item.type === 'link') {
      ...
      items.push({ label: '编辑提醒', icon: menuIcons.edit, onClick: () => editReminderItem(item) })
    }
  ...
  const addReminderItem = async () => {
    const result = await window.api.openReminderSetup({})
    if (!result) return
    addItem({ id: genId('rem'), type: 'reminder', title: result.title, datetime: result.datetime, sortOrder: config.items.length })
  }
  ...
  const editReminderItem = async (item: Extract<GridItem, { type: 'reminder' }>) => {
    const result = await window.api.openReminderSetup({ title: item.title, datetime: item.datetime })
    if (!result) return
    updateItem(item.id, { title: result.title, datetime: result.datetime, triggered: false } as any)
    showToast('提醒已更新')
  }
  ...
  const pinToDesktop = async (id: string, position?: { x: number; y: number }) => {
    try {
      ...
      if (item?.type === 'reminder') {
        await window.api.createReminderBar(id, position)
      } else {
  ...
  const handleCardClick = async (item: GridItem) => {
    ...
    if (isHoldButton(item)) {
      try {
        if (holdingIdRef.current === item.id) await releaseHold(item.id)
        await pressHold(item)
        showToast(`长按：${item.title}`)
        window.setTimeout(() => {
          if (holdingIdRef.current === item.id) releaseHold(item.id).catch(console.error)
        }, 800)
      } catch (error) {
        showToast('长按失败')
        console.error(error)
  ...
  if (isDialogMode) {
    return (
      <div className="dialog-window-page" data-dialog-type={dialogPayload?.dialogType || 'unknown'}>
